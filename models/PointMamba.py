...
class BiPixelMambaLayer(nn.Module):
    ...
    def forward(self, x, depth):
        ...
        x_div = x_div.permute(0, 1, 3, 2).contiguous()  # [B, C, patch_size, num_patches]

        codes = torch.arange(num_patches, device=x.device)

        order = codes.clone()

        perm = torch.randperm(num_patches, device=x.device)
        shuffled_order = order[perm]

        inverse = torch.zeros_like(shuffled_order)
        inverse[perm] = torch.arange(num_patches, device=x.device)

        x_div_shuffled = x_div[:, :, :, shuffled_order]  # [B, C, patch_size, num_patches]

        x_flat = x_div_shuffled.reshape(B, C, -1)  # 展平为 [B, C, patch_size * num_patches_shuffled]
        x_flat = x_flat.transpose(1, 2)  # [B, L_padded, C]

        x_norm = self.norm(x_flat)

        x_mamba = self.mamba_forw(x_norm)
        x_out = x_mamba
        x_out = x_out.transpose(1, 2).reshape(B, C, patch_size, num_patches)
        x_out = x_out[:, :, :, inverse]

        x_out = x_out.permute(0, 1, 3, 2).contiguous().reshape(B, C, L_padded)
